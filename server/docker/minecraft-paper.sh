...
read input1
if [ "$input1" == "" ]; then
sudo docker run -e EULA=TRUE -e TYPE=PAPER -e ENABLE_AUTOPAUSE=TRUE --memory "4g" -d -p 25565:25565 -v $PWD/data:/data --name mc itzg/minecraft-server
else
sudo docker run -e VERSION="$input1" -e TYPE=PAPER -e EULA=TRUE -e ENABLE_AUTOPAUSE=TRUE --memory "4g" -d -p 25565:25565 -v $PWD/data:/data --name mc itzg/minecraft-server
fi
fi
case $1 in
start)
